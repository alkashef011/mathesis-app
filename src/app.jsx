...
const TOPIC_KEYWORDS = {
  infinity:    ["infinity","infinite","∞","boundless","endless","cantor","aleph","limit"],
  primes:      ["prime","primes","prime number","sieve","eratosthenes","divisible","factor"],
  geometry:    ["geometry","geometric","shape","angle","triangle","circle","euclid","line","point","polygon"],
  algebra:     ["algebra","equation","variable","solve","unknown","polynomial","quadratic","expression"],
  patterns:    ["pattern","sequence","series","fibonacci","rule","structure","recurrence"],
  symmetry:    ["symmetry","symmetric","rotation","reflection","group","invariant","transformation"],
  calculus:    ["calculus","derivative","integral","limit","rate of change","newton","leibniz","differentiat"],
  euler:       ["euler","e^iπ","identity","eiπ","beautiful equation","e to the i","euler's formula"],
  fractals:    ["fractal","self-similar","mandelbrot","sierpinski","dimension","recursive","self similar"],
  topology:    ["topology","topological","donut","torus","klein","möbius","moebius","deform","continuous"],
  probability: ["probability","random","chance","likelihood","statistics","expected value","distribution","odds"],
  goldenratio: ["golden ratio","phi","φ","1.618","golden","golden spiral","divine proportion"],
  pi:          ["π","pi","3.14159","3.14","circumference","diameter","circle constant"],
  complex:     ["complex number","imaginary","i²","√-1","imaginary unit","real and imaginary","argand"],
  set_theory:  ["set theory","set","subset","union","intersection","empty set","cantor","cardinality","element of"],
};
...
const STARTER_PROMPT = `Begin the journey. Greet the user with warmth and genuine excitement. Ask one single opening question that reveals their relationship with mathematics and what kind of explorer they are. Keep it brief — no more than a short paragraph or two. Leave space. Make it feel like the start of something. No branches or challenges yet.`;

const RETURNING_STARTER = (summary, session) =>
  `The user is returning for session ${session}. Journey summary so far: "${summary}". Welcome them back warmly in a sentence or two — reference something specific from their journey, then ask one question to pick up the thread. Brief, energised, personal. No branches or challenges yet.`;
...
function parseSegments(text) {
  const sparkRe = /\[SPARK\s+type="([^"]+)"\s+label="([^"]+)"\]([\s\S]*?)\[\/SPARK\]/g;
  const segments = [];
  let last = 0;
  let m;
...
const SPARK_META = {
  ...
  story:     { icon:"◈", accent:"#e8c97a", bg:"#131008", border:"#e8c97a20", labelColor:"#c8a85a" },
...
function SparkCard({ sparkType, label, content }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        fontSize:"16px", color: meta.accent, opacity:0.75,
      ...
      <div style={{ flex:1 }}>
        <div style={{
          ...
          color: meta.labelColor, opacity:0.8,
...
function FloatingSymbols() {
  const syms = useRef(Array.from({ length: 20 }, (_, i) => ({
    ...
    <div style={{ position:"fixed", inset:0, pointerEvents:"none", overflow:"hidden", zIndex:0 }}>
      {syms.map(s => (
        <div key={s.id} style={{
          ...
          fontSize:`${s.size}px`, opacity:s.op, color:"#e8c97a",
...
function ConstellationMap({ discovered, onClose }) {
  ...
  return (
    <div
      style={{ position:"fixed", inset:0, zIndex:60, display:"flex", alignItems:"center", justifyContent:"center", background:"rgba(4,3,2,0.92)", animation:"fadeIn 0.25s ease" }}
      onClick={onClose}
    >
      <div onClick={e => e.stopPropagation()} style={{
        background:"linear-gradient(160deg,#0d0b08,#111009)",
        border:"1px solid #e8c97a1a", borderRadius:"3px",
        padding:"28px 28px 24px", maxWidth:"500px", width:"92vw",
        boxShadow:"0 0 80px #000c",
      }}>
        {/* Header */}
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"18px" }}>
          <div>
            <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"19px", letterSpacing:"0.18em", color:"#c8b87a", fontWeight:300 }}>
              YOUR CONSTELLATION
            </div>
            <div style={{ fontSize:"11px", color:"#4a4030", letterSpacing:"0.1em", fontFamily:"'Cormorant Garamond',serif", marginTop:"3px" }}>
              {discoveredCount} of {CONSTELLATION_NODES.length} realms discovered
            </div>
          </div>
        ...
        {/* Progress bar */}
        <div style={{ height:"2px", background:"#ffffff06", borderRadius:"1px", marginBottom:"20px" }}>
          <div style={{ height:"100%", width:`${(discoveredCount / CONSTELLATION_NODES.length) * 100}%`, background:"linear-gradient(90deg,#e8c97a44,#e8c97a88)", borderRadius:"1px", transition:"width 0.6s ease" }}/>
        </div>
          ...
          {CONSTELLATION_NODES.map(n => {
            ...
            return (
              <g key={n.id} onMouseEnter={() => setHovered(n.id)} onMouseLeave={() => setHovered(null)} style={{ cursor:"default" }}>
                {lit && <circle cx={cx} cy={cy} r={hov ? 16 : 11} fill="#e8c97a" fillOpacity={0.07} />}
                <circle cx={cx} cy={cy}
                  r={lit ? (hov ? 5.5 : 4) : 2.5}
                ...
                {lit && (
                  <text x={cx} y={cy - 11} textAnchor="middle"
                    fontSize={hov ? "9.5" : "8"} fill={hov ? "#e8c97a" : "#a09070"}
        ...
        <div style={{ minHeight:"52px", marginTop:"14px" }}>
          {hovered && (() => {
            ...
            return n ? (
              <div style={{ padding:"11px 14px", background:"#ffffff04", border:"1px solid #e8c97a12", borderRadius:"2px", animation:"fadeIn 0.2s ease" }}>
                <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"14px", color: discovered.has(n.id) ? "#e8c97a" : "#5a5040", marginBottom:"3px" }}>{n.label}</div>
                <div style={{ fontFamily:"'EB Garamond',serif", fontSize:"12.5px", color:"#6a6050", fontStyle:"italic" }}>{n.desc}</div>
                {!discovered.has(n.id) && <div style={{ fontSize:"10px", color:"#3a2a10", marginTop:"4px", letterSpacing:"0.08em" }}>not yet discovered</div>}
              </div>
            ) : null;
          })()}
        </div>
        ...
        <div style={{ marginTop:"14px", display:"flex", gap:"20px" }}>
          {[["#e8c97a","discovered"],["#2a2010 border:#4a4030","awaiting"]].map(([col, label]) => (
            <div key={label} style={{ display:"flex", alignItems:"center", gap:"6px" }}>
              <div style={{ width:"7px", height:"7px", borderRadius:"50%", background: label==="discovered"?"#e8c97a":"#2a2010", border: label==="awaiting"?"0.6px solid #4a4030":"none" }}/>
              <span style={{ fontSize:"10px", color:"#4a4030", fontFamily:"'Cormorant Garamond',serif", letterSpacing:"0.08em" }}>{label}</span>
            </div>
          ))}
        </div>
      </div>
...
function ChallengeCard({ challenge, onSubmit, onDismiss }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"11px", letterSpacing:"0.28em", color:"#e8c97a77", marginBottom:"14px" }}>
        ✦ CHALLENGE
      </div>
      <div style={{ fontFamily:"'EB Garamond',serif", fontSize:"16.5px", lineHeight:"1.75", color:"#c8b87a", marginBottom:"20px", paddingRight:"16px" }}>
        {challenge}
      </div>
      ...
      <textarea
        ...
        disabled={submitted}
        style={{
          width:"100%", background:"#ffffff04", border:"1px solid #e8c97a14",
          borderRadius:"2px", padding:"12px 14px", color:"#ddd5c0",
          fontFamily:"'EB Garamond',serif", fontSize:"15px", lineHeight:"1.65",
          resize:"vertical", outline:"none", caretColor:"#e8c97a",
      ...
      <div style={{ marginTop:"14px", display:"flex", gap:"10px", alignItems:"center" }}>
        <button
          ...
          style={{
            background:"transparent", border:"1px solid #e8c97a33",
            color:"#e8c97a", padding:"9px 24px",
            fontFamily:"'Cormorant Garamond',serif", fontSize:"13px",
            letterSpacing:"0.15em", cursor: (!attempt.trim() || submitted) ? "not-allowed" : "pointer",
        ...
        <button onClick={onDismiss} style={{ background:"none", border:"1px solid #ffffff08", color:"#3a3020", padding:"9px 16px", fontFamily:"'Cormorant Garamond',serif", fontSize:"12px", letterSpacing:"0.1em", cursor:"pointer" }}>
          not now
        </button>
      </div>
    </div>
...
function BranchCard({ branch, onChoose }) {
  ...
  return (
    <div style={{ margin:"20px 0 28px", animation:"fadeSlideIn 0.5s ease" }}>
      <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"11px", letterSpacing:"0.24em", color:"#8a7a5a", marginBottom:"16px" }}>
        ✦ TWO PATHS LIE AHEAD — CHOOSE YOUR DIRECTION
      </div>
      <div style={{ display:"flex", gap:"12px", flexWrap:"wrap" }}>
        {[branch.a, branch.b].map((path, i) => (
          <button key={i} onClick={() => handle(path, i)} style={{
            ...
            border: chosen === i ? "1px solid #e8c97a44" : "1px solid #ffffff0e",
            color: chosen === i ? "#e8c97a" : chosen !== null ? "#4a4030" : "#9a9080",
            fontFamily:"'EB Garamond',serif", fontSize:"14.5px",
            lineHeight:"1.55", cursor: chosen !== null ? "default" : "pointer",
            textAlign:"left", transition:"all 0.35s ease", borderRadius:"2px",
          }}>
            <div style={{ fontSize:"22px", marginBottom:"10px", opacity: chosen === i ? 0.9 : 0.4, transition:"all 0.3s" }}>
              {i === 0 ? "⟨" : "⟩"}
            </div>
            {path}
          </button>
  ...
  return (
    <div style={{ display:"flex", alignItems:"center", gap:"12px", padding:"14px 0" }}>
      <div style={{ display:"flex", gap:"5px" }}>
        {[0,1,2].map(i => (
          <div key={i} style={{
            width:"4.5px", height:"4.5px", borderRadius:"50%", background:"#e8c97a",
    ...
    <div style={{
      ...
      {!isUser && (
        <div style={{
          ...
          background: isEval ? "linear-gradient(135deg,#c8b87a1a,#c8b87a30)" : "linear-gradient(135deg,#e8c97a14,#e8c97a28)",
          border: `1px solid ${isEval ? "#c8b87a33" : "#e8c97a2a"}`,
          display:"flex", alignItems:"center", justifyContent:"center",
          fontSize: isEval ? "12px" : "14px", flexShrink:0,
          marginRight:"12px", marginTop:"4px", color:"#e8c97a",
        }}>
          {isEval ? "✦" : "∞"}
        </div>
      )}
      ...
      {isUser ? (
        ...
        <div style={{ maxWidth: hasSparks ? "85%" : "79%", minWidth:"40%" }}>
          {segments.map((seg, i) => {
            if (seg.type === "spark") {
              return <SparkCard key={i} sparkType={seg.sparkType} label={seg.label} content={seg.content} />;
            }
            ...
            return (
              <div key={i} style={{
                ...
                border: isEval ? "1px solid #c8b87a1a" : "1px solid #ffffff0c",
                color:"#ddd5c0", fontSize:"15.5px", lineHeight:"1.8",
                letterSpacing:"0.01em",
                fontFamily:"'Palatino Linotype','Book Antiqua',Palatino,serif",
                whiteSpace:"pre-wrap", wordBreak:"break-word",
...
export default function Mathesis() {
  ...
  const [newlyLit, setNewlyLit]               = useState(null);
  const [pingVisible, setPingVisible]         = useState(false);

  const messagesEndRef   = useRef(null);
  const inputRef         = useRef(null);
  const conversationRef  = useRef([]);
  const journeyRef       = useRef({ messages:[], discovered:["numbers"], summary:"", sessions:0 });
  ...
  const callClaude = useCallback(async (conv, systemOverride) => {
    ...
    try {
      ...
      if (data.error) {
        ...
        return "The cosmos paused for a moment. Let us try again — what were you thinking?";
      }
      return data.content?.map(b => b.text || "").join("") || "...";
    } catch (e) {
      console.error("Fetch error:", e);
      return "Something in the cosmos shifted. Let us try again — what were you thinking?";
    } finally {
      setIsThinking(false);
    }
  }, [sanitizeConv]);
    ...
    setDiscovered(prev => {
      ...
      found.forEach(t => {
        if (!next.has(t)) {
          ...
          setNewlyLit(t);
          setPingVisible(true);
          setTimeout(() => { setNewlyLit(null); setPingVisible(false); }, 3500);
        }
      });
      if (changed) {
  ...
  const beginJourney = async () => {
    ...
    const initText = hasHistory && journeySummary
      ...
      <div style={{ minHeight:"100vh", background:"#0a0906", position:"relative" }}>
        ...
        {screen === "intro" && (
          <div style={{ position:"fixed", inset:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", zIndex:10, padding:"40px 20px", background:"#0a0906" }}>
            <div style={{ textAlign:"center", maxWidth:"560px", animation:"introFadeIn 1.3s ease forwards" }}>
              ...
              <h1 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"clamp(44px,8vw,70px)", fontWeight:300, letterSpacing:"0.16em", color:"#f0e4bc", lineHeight:1, marginBottom:"8px" }}>
                MATHESIS
              </h1>
              <p style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"12px", letterSpacing:"0.36em", color:"#5a4a2a", marginBottom:"42px", fontWeight:300 }}>
              ...
              {isReturning ? (
                <>
                  ...
                  {journeySummary && (
                    <p style={{ fontFamily:"'EB Garamond',serif", fontSize:"14.5px", lineHeight:"1.8", color:"#6a6050", marginBottom:"12px", maxWidth:"420px", margin:"0 auto 12px" }}>
                      {journeySummary}
                    </p>
                  )}
                  <p style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"12px", letterSpacing:"0.12em", color:"#3a3020", marginBottom:"44px" }}>
                    {discovered.size} realms discovered · session {sessionCount + 1}
                  </p>
                </>
              ) : (
                <>
                  <p style={{ fontFamily:"'EB Garamond',serif", fontSize:"19px", lineHeight:"1.85", color:"#9a9080", marginBottom:"12px", fontStyle:"italic" }}>
                    Mathematics is not a subject to be learned.
                  </p>
                  <p style={{ fontFamily:"'EB Garamond',serif", fontSize:"19px", lineHeight:"1.85", color:"#9a9080", marginBottom:"46px", fontStyle:"italic" }}>
                    It is a world to be discovered — one beautiful idea at a time.
                  </p>
                </>
              )}
              ...
              <p style={{ marginTop:"28px", fontSize:"11px", letterSpacing:"0.12em", color:"#2a2010", fontFamily:"'Cormorant Garamond',serif" }}>
        ...
        {screen === "journey" && (
          <div style={{ position:"relative", zIndex:1, display:"flex", flexDirection:"column", height:"100vh", maxWidth:"800px", margin:"0 auto", padding:"0 22px" }}>
            ...
            {/* Header */}
            <div style={{ padding:"15px 0 13px", borderBottom:"1px solid #ffffff06", display:"flex", alignItems:"center", justifyContent:"space-between", flexShrink:0 }}>
              <div style={{ display:"flex", alignItems:"center", gap:"13px" }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize:"9.5px", letterSpacing:"0.14em", color:"#2a2010", fontFamily:"'Cormorant Garamond',serif" }}>YOUR JOURNEY · YOUR PACE · YOUR DISCOVERY</div>
                </div>
              </div>
              ...
              <button className="icon-btn" onClick={() => setShowConstellation(true)} style={{ position:"relative" }}>
                ...
                <span style={{ fontFamily:"'Cormorant Garamond',serif", letterSpacing:"0.08em" }}>{discovered.size}/{CONSTELLATION_NODES.length} realms</span>
              </button>
            </div>
            ...
            <div style={{ padding:"12px 0 18px", borderTop:"1px solid #ffffff06", flexShrink:0 }}>
              ...
              <p style={{ textAlign:"center", fontSize:"10px", color:"#1e1608", marginTop:"8px", letterSpacing:"0.07em", fontFamily:"'Cormorant Garamond',serif" }}>
        ...
        {showConstellation && (
          <ConstellationMap discovered={discovered} onClose={() => setShowConstellation(false)} />
        )}
      </div>
    </>
  );
